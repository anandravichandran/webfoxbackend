...
@function_routes.route('/handleDownload', methods=['POST'])
def handle_download():
    data = request.json
    download_url = data.get('downloadUrl')
    file_name = data.get('fileName')

    print(f"Received download URL: {download_url}, file name: {file_name}")

    # Handle the download URL here (e.g., log it, process it, etc.)
    return jsonify({'status': 'success', 'message': 'Download URL received'})